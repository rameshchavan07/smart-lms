...
const regex = /<Download className="w-5 h-5" \/>\s*<\/a>\s*\{user\?\.role === 'TEACHER' && \(\s*<button\s*<\/div>\s*\)\s*:\s*enrolledStudents\.length === 0 \? \(/;

const correct = `<Download className="w-5 h-5" />
                    </a>
                    {user?.role === 'TEACHER' && (
                      <button 
...
if (regex.test(content)) {
  fs.writeFileSync(file, content.replace(regex, correct));
