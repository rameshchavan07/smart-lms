    ...
    <div className={`flex h-screen overflow-hidden ${isDark ? 'dark' : ''}`} style={{ background: 'var(--bg)', color: 'var(--text-primary)' }}>
      ...
      <aside className={`sidebar ${isMobileOpen ? 'open' : ''}`}>
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-white/5">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shadow-brand flex-shrink-0" style={{ background: 'var(--brand-500)' }}>
        ...
        <nav className="flex-1 overflow-y-auto hide-scrollbar py-3">
          <p className="text-white/25 text-[10px] font-semibold uppercase tracking-widest px-5 mb-2">Management</p>
          {navItems.map((item) => {
          ...
          <div className="mx-3 mt-4">
            <Link to="/admin/courses" className="w-full btn btn-primary btn-sm justify-start gap-2 text-center flex items-center">
